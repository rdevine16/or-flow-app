...
// Redesigned: Exception-based list instead of surgeon x procedure matrix
// Shows only overrides that exist, with Add Override flow
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await supabase
        ...
        .eq('procedure_type_id', selectedProcedureId)
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <select
                    ...
                    onChange={(e) => setFilterSurgeonId(e.target.value)}
                  ...
                  <span className="text-sm text-slate-500">
                    {groupedOverrides.length} override{groupedOverrides.length !== 1 ? 's' : ''}
                  </span>
                </div>
                <button
              ...
              {groupedOverrides.length === 0 ? (
                <div className="bg-white border border-slate-200 rounded-xl p-8 text-center">
                  ...
                  <h3 className="text-lg font-medium text-slate-900 mb-1">No Overrides Yet</h3>
                  <p className="text-slate-500 mb-4">
                    All surgeons are using default procedure costs.
                  </p>
                  <button
                    onClick={openAddPanel}
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {groupedOverrides.map((override) => (
                        <tr key={`${override.surgeonId}-${override.procedureId}`} className="hover:bg-slate-50">
                          <td className="px-4 py-3">
                            <span className="font-medium text-slate-900">{override.surgeonName}</span>
                          </td>
                          <td className="px-4 py-3">
                            <span className="text-slate-600">{override.procedureName}</span>
                          </td>
                          <td className="px-4 py-3">
                            <span className="inline-flex items-center px-2 py-1 bg-blue-100 text-blue-700 text-xs font-medium rounded-full">
                              {override.overrideCount} categor{override.overrideCount !== 1 ? 'ies' : 'y'}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-right">
                            <div className="flex items-center justify-end gap-1">
                              ...
                              {deleteConfirm?.surgeonId === override.surgeonId && deleteConfirm?.procedureId === override.procedureId ? (
              ...
              <div className="flex items-start gap-3 p-4 bg-slate-50 border border-slate-200 rounded-xl">
                ...
                <div className="text-sm text-slate-600">
                  ...
                  <p>When a case is created, the system checks for surgeon-specific cost overrides first. If none exist, the default procedure costs are used.</p>
                </div>
              </div>
            </div>
          )}

// app/admin/settings/flag-rules/page.tsx
// Admin Flag Rule Templates — manages global default flag rules (facility_id IS NULL).
// Mirrors the facility flag rules page with inline editing, category filter,
// custom rule builder, archive/restore, and audit logging.
...
import type { FlagRule, Severity, ThresholdType, Operator, ComparisonScope, CustomRuleFormState } from '@/types/flag-settings'
...
export default function AdminFlagRulesPage() {
  ...
  // Data: active template rules (is_active = true, facility_id IS NULL)
  const { data: rules, loading, error, setData: setRules } = useSupabaseQuery<FlagRule[]>(
    async (sb) => {
      const result = await flagRulesDal.listActiveTemplates(sb)
  ...
  // Data: archived template rules (is_active = false, facility_id IS NULL)
  const {
    data: archivedRules,
    setData: setArchivedRules,
  } = useSupabaseQuery<FlagRule[]>(
    async (sb) => {
      const result = await flagRulesDal.listArchivedTemplates(sb)
      if (result.error) throw new Error(result.error.message)
      return result.data
    },
    { enabled: isGlobalAdmin }
  ...
  // =====================================================
  // BUILDER SUBMIT — Create custom template rule via DAL
  // =====================================================

  const handleBuilderSubmit = useCallback(
    async (form: CustomRuleFormState) => {
      const result = await flagRulesDal.createCustomTemplateRule(supabase, form)
      ...
      flagRuleAudit.created(supabase, newRule.id, newRule.name, null, {
  ...
  const handleArchive = useCallback(
    async (rule: FlagRule) => {
      ...
      try {
        ...
        flagRuleAudit.archived(supabase, rule.id, rule.name, null)
    ...
    [supabase, showToast, setRules, setArchivedRules, markSaving]
  )
  ...
  const handleRestore = useCallback(
    async (rule: FlagRule) => {
      ...
      try {
        ...
        flagRuleAudit.restored(supabase, rule.id, rule.name, null)
  ...
  // No dynamic cost-category metrics for templates (those are facility-specific)
      ...
      <DashboardLayout>
        <Container className="py-8">
          ...
          <p className="text-slate-500 mb-6">
            Configure default flag rules for new facilities.
          </p>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 max-w-xl leading-relaxed">
              Configure default flag rules that are seeded to newly created facilities.
              Built-in rules provide standard OR metrics. Add custom rules from any metric.
            </p>
          </div>
        ...
        <div className="mb-5 p-3 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex gap-2">
            ...
            <p className="text-sm text-blue-700">
              Changes apply to new facilities only. Existing facilities manage their own flag rules independently.
            </p>
          </div>
        </div>
            ...
            <>
              {archivedList.length > 0 ? (
                ...
                <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50">
                  <p className="text-sm text-slate-500 mb-1">No archived template rules</p>
                  <p className="text-xs text-slate-400">
                    Archived rules will appear here when you archive custom template rules.
                  </p>
                </div>
              )}
            </>
            ...
            <div>
              ...
              {customRules.length > 0 ? (
                ...
                <div className="text-center py-8 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50">
                  <p className="text-sm text-slate-500 mb-1">No custom template rules yet</p>
          ...
          {filterCategory !== 'archived' &&
            ...
            customRules.length === 0 && (
              <div className="text-center py-12 text-sm text-slate-400">
                No template rules match the selected category.
              </div>
            )}
        </div>

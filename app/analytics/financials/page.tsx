...
function calculateCaseProfit(
...
): { profit: number; reimbursement: number; orCost: number } | null {
  ...
  let reimbursement = reimbursements.find(
    r => r.procedure_type_id === procedure.id && r.payer_id === caseData.payer_id
  )?.reimbursement
  
  if (!reimbursement) {
    reimbursement = reimbursements.find(
  ...
  return { profit, reimbursement, orCost }
}
...
export default function FinancialsAnalyticsPage() {
  ...
  const metrics = useMemo(() => {
    ...
    const casesWithProfit = cases
      ...
      .filter((c): c is CaseWithFinancials & { profit: number; reimbursement: number; orCost: number; duration: number } => c !== null)
    ...
    const outlierDetails: OutlierCase[] = outlierCases.map(c => {
      const procedureStats = procedureMap.get(c.procedure_type_id || '')
      const expectedDuration = procedureStats 
        ? procedureStats.reduce((sum, pc) => sum + (pc.duration || 0), 0) / procedureStats.length
        : c.duration || 0
              ...
              {activeTab === 'outliers' && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <MetricCard 
                      title="Due to Time" 
                      value={metrics.outlierDetails.filter(c => c.issue === 'time').length}
                      subtitle="Excessive OR time"
                    />
                    <MetricCard 
                      title="Threshold" 
                      value={formatCurrency(metrics.outlierThreshold)}
                      subtitle="Outlier cutoff"
                    />
                  </div>
                  ...
                  {metrics.outlierDetails.length > 0 ? (
                    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                      <div className="px-6 py-4 border-b border-slate-200">
                        ...
                        <p className="text-sm text-slate-500">Sorted by largest gap from expected</p>
                      </div>
                      <div className="overflow-x-auto">
                        <table className="w-full">
                          <thead className="bg-slate-50">
                            <tr>
                              ...
                              <th className="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase">Issue</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-slate-100">

...
import { getAvailableTextColors, getSuggestedColors } from '@/lib/scanner/color-extractor'
...
// Enhanced findHardcodedColors for route.ts
// Add this to your app/api/refactor/scan/route.ts


// CORRECTED findHardcodedColors for route.ts
// Replace your entire findHardcodedColors function with this
...
function findHardcodedColors(
  ...
  // Get available colors once for all issues
  const availableColors = getAvailableTextColors()
  
  const hardcodedColorPattern = /(text)-(slate|gray|zinc|neutral|stone|red|orange|amber|yellow|lime|green|emerald|teal|cyan|sky|blue|indigo|violet|purple|fuchsia|pink|rose)-(\d{2,3})/
  ...
  lines.forEach((line, index) => {
    ...
    for (const match of matches) {
      ...
      const colorType = match[1] as 'text'
      ...
      // Check if this is a conditional color (ternary)
      ...
      // Detect condition variable using inline function
      const conditionVar = detectConditionVariableInline(line)

      // Get suggested colors from the imported function
      const activeSuggestionsRaw = getSuggestedColors(activeColor, availableColors, 'active')
      const inactiveSuggestionsRaw = inactiveColor 
        ? getSuggestedColors(inactiveColor, availableColors, 'inactive')
        : []

      // Map to the format UI expects
      const activeSuggestions = activeSuggestionsRaw.map(c => ({
      ...
      const inactiveSuggestions = inactiveSuggestionsRaw.map(c => ({
        className: c.className,
        displayName: c.displayName,
        family: c.family,
        shade: c.shade
      ...
      issues.push({
        ...
        risk: 'safe',
        description: `Replace hardcoded ${isConditional ? 'conditional ' : ''}colors with design system tokens`,
        beforeCode: line.trim(),
        afterCode: '', // Will be generated by UI
        context,
        warnings: [
          isConditional 
            ? `Found conditional colors: ${activeColor} / ${inactiveColor}`
            : `Found hardcoded color: ${activeColor}`,
          `${activeSuggestions.length} suggested alternatives available`
        ],
        metadata: {
          colorType: 'text',
          isConditional,
  ...
  // Inline helper function (no conflicts!)

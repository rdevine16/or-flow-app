...
-- The advisor migration set search_path = '' which is too restrictive.
...
DECLARE
  ...
  alter_cmd TEXT;
BEGIN
  -- Fix all public functions that have search_path set but NOT to 'public'
  FOR func IN
    SELECT
    ...
    WHERE n.nspname = 'public'
      ...
      AND EXISTS (
        ...
        WHERE conf LIKE 'search_path=%'
          AND conf NOT LIKE 'search_path=%public%'
      )
    ORDER BY p.proname
  LOOP
    alter_cmd := format(

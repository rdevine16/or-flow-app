-- Add category field to flag rule breakdown and recent flagged cases
-- in the get_flag_analytics RPC, enabling client-side financial flag gating.
...
BEGIN
  ...
  SELECT COALESCE((
    SELECT jsonb_agg(jsonb_build_object(
      ...
      'turnover', d.turnover,
      'delay', d.delay_count,
      'financial', d.financial_count,
      'quality', d.quality_count,
      'total', d.total
    ) ORDER BY d.day_num)
    FROM (
      SELECT
        ...
        count(*) FILTER (WHERE cf.flag_type = 'delay') AS delay_count,
        count(*) FILTER (WHERE cf.flag_type = 'threshold' AND fr.category = 'financial') AS financial_count,
        count(*) FILTER (WHERE cf.flag_type = 'threshold' AND fr.category = 'quality') AS quality_count,
        count(*) AS total
      FROM case_flags cf
      JOIN cases c ON c.id = cf.case_id
      JOIN case_statuses cs ON cs.id = c.status_id
  ...
  -- ================================================================
  -- SECTION 5: Flag Rule Breakdown (now includes category)
  -- ================================================================
  ...
  SELECT COALESCE((
    ...
    FROM (
      ...
      WHERE cf.facility_id = p_facility_id
        ...
        AND c.is_draft = false
        AND c.is_excluded_from_metrics = false
      GROUP BY fr.name, fr.severity, fr.category
    ) rb
  ...
  SELECT COALESCE((
    ...
    SELECT jsonb_agg(jsonb_build_object(
      ...
      'flaggedCases', sc.flagged_cases,
      'flags', sc.flag_count,
      'rate', round(sc.flagged_cases::numeric / sc.total_cases * 100, 1),
      'prevRate', CASE WHEN sp.total_cases > 0
        THEN round(sp.flagged_cases::numeric / sp.total_cases * 100, 1)
        ELSE NULL END,
      'trend', CASE
  ...
  -- ================================================================
  -- SECTION 9: Recent Flagged Cases (now includes category per flag)
  -- ================================================================
      ...
      SELECT
        ...
        (
          SELECT jsonb_agg(jsonb_build_object(
            ...
            'category', COALESCE(fr2.category, 'delay')
          ))
          FROM case_flags cf2

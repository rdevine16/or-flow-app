...
// Phase 6: urgent badge, "View all" link, lighter row styling.
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <h2 className="text-base font-semibold text-slate-900">Needs Attention</h2>
          {urgentCount > 0 && (
            <span className="text-xs font-semibold text-white bg-red-500 px-1.5 py-0.5 rounded-full min-w-[20px] text-center">
              {urgentCount}
            </span>
          )}
          {alerts.length > 0 && (
...
function AlertItem({ alert }: { alert: DashboardAlert }) {
  ...
  return (
    <Link
      href={alert.linkTo}
      className={`flex items-start gap-3 py-2.5 px-3 -mx-3 rounded-lg transition-colors group ${colors.bg}`}
    >
      {/* Priority dot */}

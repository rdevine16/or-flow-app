        ...
        {cases.map((c) => (
          <div key={c.id} className="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-slate-50 transition-colors">
            <div className="col-span-2">
              <span className="font-semibold text-slate-900">{c.case_number}</span>
            </div>
            <div className="col-span-2">
              <span className="inline-flex items-center gap-1.5 text-slate-600">
                ...
                {c.or_rooms?.name || 'Unassigned'}
              </span>
            </div>
            <div className="col-span-4">
              <span className="text-slate-700">{c.procedure_types?.name || 'Not specified'}</span>
            </div>
            <div className="col-span-2">
              <Badge variant={getStatusVariant(c.case_statuses?.name)}>
                {formatStatus(c.case_statuses?.name)}
              </Badge>
            </div>

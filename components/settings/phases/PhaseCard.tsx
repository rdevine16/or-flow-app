...
interface PhaseCardProps {
  ...
  /** Where the drop indicator line should appear: 'before' | 'after' | null */
...
/** Simplified overlay card shown while dragging */
export function PhaseCardOverlay({ phase }: { phase: PhaseCardData }) {
  const colorConfig = resolveColorKey(phase.color_key)

  return (
    <div className="flex items-center gap-3 px-4 py-3 bg-white border border-blue-300 rounded-lg shadow-xl ring-2 ring-blue-200 w-[400px] opacity-95">
      <GripVertical className="w-4 h-4 text-slate-400" />
      <div className={`w-4 h-4 rounded-full ${colorConfig.swatch} ring-1 ring-black/10`} />
      <span className="text-sm font-medium text-slate-900">{phase.display_name}</span>
    </div>
  )
}

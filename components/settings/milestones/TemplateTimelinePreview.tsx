      ...
      {renderList.map((item, idx) => {
        switch (item.type) {
          ...
          case 'boundary-connector':
            return (
              <ReadOnlyBoundaryConnector
                key={`bc-${item.endsPhase.id}-${item.startsPhase.id}`}
                item={item}
              />
            )
          case 'sub-phase':
...
function ReadOnlyBoundaryConnector({ item }: { item: BoundaryConnectorItem }) {
    ...
    <div className="flex items-center justify-center py-0.5">
      <div className="flex flex-col items-center gap-0">
        ...
        <div className="flex items-center gap-1 px-2 py-[1px]">
          ...
          <span className="text-[7.5px] text-slate-300">&rarr;</span>
          <span
            className="text-[7.5px] font-bold uppercase tracking-wide"

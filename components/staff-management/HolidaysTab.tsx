// components/staff-management/HolidaysTab.tsx
// Holidays + closures management tab embedded in Staff Management page.
// Reuses useFacilityClosures hook and patterns from /settings/closures.
...
export function HolidaysTab({ facilityId }: HolidaysTabProps) {
  const {
    holidays,
    closures,
    loading,
  ...
  useEffect(() => {
    fetchHolidays()
    fetchClosures()
  }, [fetchHolidays, fetchClosures])

  // Handlers
  const handleCreateHoliday = useCallback(
    async (input: CreateHolidayInput) => {
      const result = await createHoliday(input)
      if (result) setHolidayDialogOpen(false)
    },
    [createHoliday],
  )
  ...
  const handleCreateClosure = useCallback(
    async (input: CreateClosureInput) => {
      const result = await createClosure(input)
      if (result) setClosureDialogOpen(false)
    },
    [createClosure],
  )
  ...
  return (
    <>
      {/* Error Display */}
      {error && (
        <div className="flex items-center gap-3 p-4 mb-4 bg-red-50 border border-red-200 rounded-xl text-red-600">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <>
                {activeHolidays.map((holiday) => (
                  <HolidayRow
                    ...
                    onDelete={() =>
                      setDeleteTarget({ type: 'holiday', id: holiday.id, name: holiday.name })
                    }
                    loading={loading}
                  />
                ))}
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-slate-50/50">
            ...
            <Button onClick={() => setClosureDialogOpen(true)} size="sm">
              <Plus className="h-4 w-4" />
              Add Closure
            </Button>
          </div>
...
function ClosureRow({ closure, onDelete, past }: ClosureRowProps) {
  ...
  const daysUntil = Math.ceil(
    (closureDate.getTime() - todayDate.getTime()) / (1000 * 60 * 60 * 24),
  )
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 ml-4">
          <button
            onClick={onDelete}
            className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"
            aria-label={`Delete closure on ${formattedDate}`}
          >
            <Trash2 className="h-4 w-4" />
          </button>
        </div>
...
// ============================================================
// HOLIDAY DIALOG (with partial holiday support)
// ============================================================
...
function HolidayDialog({ open, onClose, onSave, editingHoliday, loading }: HolidayDialogProps) {
    ...
    if (editingHoliday) {
      ...
      setPartialCloseTime(
        editingHoliday.partial_close_time
          ? editingHoliday.partial_close_time.slice(0, 5) // "12:00:00" → "12:00"
          : '12:00',
      )
  ...
  return (
    <Modal open={open} onClose={onClose} title={editingHoliday ? 'Edit Holiday' : 'Add Holiday'}>

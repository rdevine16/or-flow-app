...
## Review Decisions (2026-02-14)

> These decisions were made during a `/review` session after examining the actual codebase. They override or refine the plan above where they conflict.

### Access Levels & Roles
- **Device reps excluded** — device reps are separate entities (facility_device_reps table), not in the `users` table. Remove all `device_rep` templates from the plan. Permission system only targets `user` and `coordinator`.
- **Coordinator templates added** — coordinator is a real access level in the DB. Gets nearly facility_admin-level defaults (all granted except `settings.manage`, `users.manage`, `audit.view`).
- **Role selector** — Two tabs (User | Coordinator) on both admin pages.

### Case Drawer Tabs
- **Match actual tabs** — The plan's tab permissions (overview, financials, milestones, implants, staff) don't match the real drawer. Replace with: `tab.case_financials`, `tab.case_milestones`, `tab.case_flags`, `tab.case_validation`. Drop non-existent tabs.

### UserContext Integration
- **Replace hardcoded checks** — All `allowedRoles`, `requiredAccess`, `canCreateCases`, `isCoordinator`, `isFacilityAdmin` replaced with `can()` calls. Single source of truth.
- **Keep isGlobalAdmin + isAdmin** — `isGlobalAdmin` needed for impersonation logic. `isAdmin` useful as bypass shorthand. Remove `canCreateCases`, `isFacilityAdmin`, `isCoordinator`.
- **Parallel fetch** — Fetch user data AND permissions concurrently in UserContext. `loading` resolves when both complete.
- **Use useSupabaseQuery** — Follow established pattern for the permissions RPC call (not raw useState/useEffect).

### Impersonation
- **Explicit facility_id** — Facility permissions page uses `effectiveFacilityId` from `useUser()`, same as all other settings pages (48 files). The `get_user_permissions` RPC is only for resolving the *current user's* own permissions, not for admin UI.

### UI Decisions
- **Toggle component (sm)** — Reuse existing `components/ui/Toggle.tsx` for matrix cells.
- **Auto-save per toggle** — Immediate upsert on each toggle change. Brief toast confirmation.
- **AccessDenied component** — In-page "You don't have permission" message with link to dashboard. No redirect.
- **Skip loading state in PermissionGuard** — By the time guards render, UserContext is already loaded.

### Navigation Placement
- **Facility admin page** — `/settings/permissions` in the Organization group (alongside Users & Roles).
- **Global admin page** — `/admin/permission-templates` in the Management group (alongside Facilities).

### Phase Structure Changes
- **Phase 4 split** — Split into 4a (navigation + route-level guards) and 4b (action buttons + component-level gating).
- **Phase 5 deferred** — RLS hardening deferred to a separate project. This implementation ships Phases 1–4 only.

### Schema & Migration
- **Single migration file** — `20260215000000_permissions_system.sql` with all tables, functions, seeds, and backfill.
- **Create SQL helpers** — `get_my_access_level()` and `get_my_facility_id()` don't exist yet; create them in Phase 1.
- **Seed all 41 permissions** — Even for features not yet built. Harmless and future-proof.
- **Sync detection** — When global admin visits templates page, show banner if facilities are missing permissions. "Push template defaults" action.

### Technical Decisions
- **DB is source of truth** — Permission keys are plain strings. No TypeScript enum. Maximum flexibility.
- **Next page load only** — Permission changes take effect on next navigation/refresh. No real-time subscription.
- **No device_rep in UserContext type** — Type stays as `'global_admin' | 'facility_admin' | 'coordinator' | 'user'`.

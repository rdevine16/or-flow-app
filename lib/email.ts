...
/**
 * Send email notification when a time-off request is approved or denied.
 * Sent to the staff member who made the request.
 */
export async function sendTimeOffReviewedEmail(
  ...
  try {
    const { data, error } = await getResendClient().emails.send({
      from: DEFAULT_FROM,
      to: [to],
      subject: `Time-Off Request ${statusLabel} — ${facilityName}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width: 480px;">
                      ...
                      <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                        <tr>
                          <td style="padding: 40px;">
                            ...
                            <p style="margin: 0 0 24px; font-size: 15px; color: #334155; line-height: 1.6;">
                              Your time-off request has been reviewed by <strong style="color: #0f172a;">${reviewerName}</strong>.
                            </p>

                            <!-- Status Badge -->
                            <div style="background: ${statusBg}; border-left: 3px solid ${statusColor}; padding: 16px 20px; margin: 0 0 28px; border-radius: 0 8px 8px 0;">
                              <p style="margin: 0 0 4px; font-size: 12px; font-weight: 600; color: #64748b; text-transform: uppercase; letter-spacing: 0.5px;">
                                Status
                              </p>
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-bottom: 24px;">
                              <tr>
                                <td style="padding: 8px 0; font-size: 13px; color: #64748b; width: 100px;">Type</td>
                                <td style="padding: 8px 0; font-size: 14px; color: #0f172a; font-weight: 500;">${typeLabel}</td>
                              </tr>
                              <tr>
                                <td style="padding: 8px 0; font-size: 13px; color: #64748b; border-top: 1px solid #f1f5f9;">Dates</td>
                                <td style="padding: 8px 0; font-size: 14px; color: #0f172a; font-weight: 500; border-top: 1px solid #f1f5f9;">${dateRange}</td>
                              </tr>
                            </table>

                            ${reviewNotes ? `
                            <!-- Review Notes -->
                            <div style="background: #f8fafc; padding: 16px 20px; margin: 0 0 24px; border-radius: 8px;">
                              <p style="margin: 0 0 4px; font-size: 12px; font-weight: 600; color: #64748b; text-transform: uppercase; letter-spacing: 0.5px;">
                                Reviewer Notes
                              </p>
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                              <tr>
                                <td align="center" style="padding: 4px 0 0;">
                                  <a href="${APP_URL}/staff-management" style="display: inline-block; background-color: #2563eb; color: #ffffff; text-decoration: none; padding: 14px 36px; border-radius: 8px; font-weight: 600; font-size: 15px;">
                                    View in ORbit
                                  </a>
                                </td>
                              </tr>
                  ...
                  <tr>
                    <td style="padding: 24px; text-align: center;">
                      <p style="margin: 0; font-size: 12px; color: #94a3b8;">
                        This notification was sent from ORbit for ${facilityName}.
                      </p>
                      <p style="margin: 8px 0 0; font-size: 12px; color: #94a3b8;">
                        &copy; ${new Date().getFullYear()} ORbit Surgical
                      </p>
                    </td>
                  </tr>
...
Your ${typeLabel} request for ${dateRange} has been ${statusLabel.toLowerCase()} by ${reviewerName}.
${reviewNotes ? `\nReviewer notes: ${reviewNotes}\n` : ''}
View details at ${APP_URL}/staff-management
...
© ${new Date().getFullYear()} ORbit Surgical
    ...
    if (error) {
      log.error('[EMAIL ERROR] Failed to send time-off reviewed email:', error)
      return { success: false, error: error.message }
    }

    return { success: true, messageId: data?.id }
  } catch (err) {
    log.error('[EMAIL ERROR] Exception sending time-off reviewed email:', err)
    return { success: false, error: err instanceof Error ? err.message : 'Unknown error' }
  }
}
